...
public class BunnyGrabber extends Task<ClientContext> {
    ...
    private int [] worldList = {472,473,474,475,476,497,498,499,500,501,502,503,504};
    ...
    public BunnyGrabber(ClientContext ctx) {
        super(ctx);
    }
    ...
    public void execute() {
        ...
        {
            System.out.println("Bunny Option 1 start");
            walkPath();
            selectTelegrab();
            castTelegrab();
            System.out.println("Bunny Option 1 end");

        }
        else if(!wineOfZammy.valid())
        {
            System.out.println("Bunny Else option start");
            walkSafe();
            worldHop();
            System.out.println("Bunny Else option end");
    ...
    private void castTelegrab() {
        ...
        if (ctx.magic.casting(TELEKINETIC_GRAB))
        {
            ...
            else if(ctx.inventory.select().count() == InventSpace && ctx.groundItems.select().id(WineID).nearest().poll().valid())
            {
                wine = false;
            ...
            {
                wine=true;
                System.out.println("success or non idk");
            }
            walkSafe();
        }
        ...
        {
            System.out.println("Casting option 3 start");
            if(needsHeal())
            {
                Heal();
            }
            ...
            Condition.sleep(10000);
            worldHop();
            System.out.println("Casting option 3 end");
        }
        else
        {
