...
public class PowerKill extends Task<ClientContext>
{
    public static final Tile[] path = {new Tile(3255, 3287, 0)};
    public int FoodID = 333; //Trout
    private int LootID = 526; //Bones
    //{2791,2790,2793}; //Cow
    //{3254}; //Guard
    private int[] NPCID = {2791,2790,2793}; //Cow
    private String NPCName = "Cow";
    private int healthToHeal = 15;


    public PowerKill(ClientContext ctx) {
        super(ctx);
    }
    ...
    public void execute() {
        ...
        if(ctx.inventory.select().id(LootID).count()>0)
        {
            buryBones();
        }
